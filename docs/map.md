...
**The six hurt the party instead.** Image `0x0abd2` is what runs on arrival, from a step, a door and a spell alike, and `+2` bit 14 is only the first of the branches it makes on that word. With none of them set it reads `+4` as an attack id and lands that entry on every character who can be reached ([combat.md](combat.md)): terrain 332, 333, 336 and 337 land entry 12, object 144 lands entry 2 and object 157 lands entry 46, which are 2 to 6, 1 to 10 and 3 to 10 health times the character's level. **Bit 0 spares whoever is shod for it.** The four terrain records carry it, and image `0x0adac` then skips any character whose feet hold item 629, RED DRAGON SKIN BOOTS.

The other branches of that word take gold, food or nuore through attack ids 15, 16 and 17, or break and destroy a piece named at `+4` through entries 43 and 1. None of the nineteen records carries those bits, so nothing in the game reaches them.
...
against `rand(100)`, which succeeds at or under the chance. Image `0x18E6E` is the caller: it returns at once where the trap is zero, and otherwise rolls and springs the trap on a failure. **What a trap does is an attack.** It queues the entry at `DS:0x96DA + 12 x number` into the pending-effect table at `DS:0xF4A`, which is the same table a monster's special attack indexes and the same record size (image `0x124D8`). The entries are in the load image and [combat.md](combat.md) reads their twelve bytes, so a trap number names a sound, an animation, a damage range, an effect mask and the flags that decide how much it takes. The 451 records carry 22 different numbers between them, which are the thirteen entries twice over, and two of the thirteen take nothing off: entry 3 is a sound and an animation, and entry 1 is the DESTROY the equipment handler reads ([combat.md](combat.md)) reached with no slot named, since the queue at `0x124d8` fills no slot offset. One cell carries each.
