...
- **Do not disable sound in the emulator.** With `sbtype=none` the game hangs on its second splash. Use the game's own switches instead. **`/NOM /NOS`** are supported code paths, and `HEADLESS_ARGS` in [cabinet/boot.js](../cabinet/boot.js) passes them by default. What each switch skips is in [audio.md](audio.md).
